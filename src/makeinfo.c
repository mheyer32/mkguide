...
#define savestring(x) \
  ((char *)strcpy((char *)xmalloc(1 + ((x) ? strlen(x) : 0)), (x) ? (x) : ""))
...
#define skip_whitespace()                                                  \
  while (input_text_offset != size_of_input_text && whitespace(curchar())) \
  input_text_offset++
...
int cm_tex(), cm_asterisk(), cm_dots(), cm_bullet(), cm_TeX(), cm_copyright(),
    cm_code(), cm_samp(), cm_file(), cm_kbd(), cm_key(), cm_ctrl(), cm_var(),
    cm_dfn(), cm_emph(), cm_strong(), cm_cite(), cm_italic(), cm_bold(),
    cm_roman(), cm_title(), cm_w(), cm_refill(), cm_titlefont();

int cm_chapter(), cm_unnumbered(), cm_appendix(), cm_top(), cm_section(),
    cm_unnumberedsec(), cm_appendixsec(), cm_subsection(),
    cm_unnumberedsubsec(), cm_appendixsubsec(), cm_subsubsection(),
    cm_unnumberedsubsubsec(), cm_appendixsubsubsec(), cm_heading(),
    cm_chapheading(), cm_subheading(), cm_subsubheading(), cm_majorheading(),
    cm_raisesections(), cm_lowersections();

/* All @defxxx commands map to cm_defun (). */
int cm_defun();

int cm_node(), cm_menu(), cm_xref(), cm_ref(), cm_ftable(), cm_vtable(),
...

{
  ...
  while ((c = getopt_long(argc, argv, "D:U:I:f:o:p:e:r:s:V", long_options,
                          &ind)) != EOF) {
    if (c == 0 && long_options[ind].flag == 0) c = long_options[ind].val;
    ...
    switch (c) {
      ...
      case 'I':
        if (!include_files_path) include_files_path = savestring(".");
      ...
      case 'f':
        if (sscanf(optarg, "%d", &fill_column) != 1) usage();
        break;
      ...
      case 'p':
        if (set_paragraph_indent(optarg) < 0) usage();
        break;

        /* User specified error level? */
      case 'e':
        if (sscanf(optarg, "%d", &max_error_level) != 1) usage();
        break;

        /* User specified reference warning limit? */
      case 'r':
        if (sscanf(optarg, "%d", &reference_warning_limit) != 1) usage();
        break;

        /* User specified footnote style? */
      case 's':
        if (set_footnote_style(optarg) < 0) usage();
...
void *xmalloc(int nbytes) {
  ...
  if (nbytes && temp == (void *)NULL) memory_error("xmalloc", nbytes);
...

{
  ...
  if (nbytes && !temp) memory_error("xrealloc", nbytes);
...

{
  ...
  sprintf(printable_string,
          "Virtual memory exhausted in %s ()!  Needed %d bytes.", callers_name,
          bytes_wanted);
...
`--amiga-39'          to convert to AmigaGuide? V39 hypertext format \n\n",
          progname, paragraph_start_indent, fill_column, max_error_level,
...
char *find_and_load(char *filename) {
  ...
  if (!fullpath) goto error_exit;

  filename = fullpath;

  file = open(filename, O_RDONLY);
  if (file < 0) goto error_exit;
...
#if defined(VMS)
  while ((n = read(file, result + count, fileinfo.st_size)) > 0) count += n;
  if (n == -1)
#else  /* !VMS */
...
void popfile() {
  ...
  if (!filestack) abort(); /* My fault.  I wonder what I did? */

  /* Make sure that commands with braces have been satisfied. */
  if (!executing_string) discard_braces();
...
void push_node_filename() {
  if (node_filename_stack_index + 1 > node_filename_stack_size) {
    if (!node_filename_stack)
      node_filename_stack =
          (char **)xmalloc((node_filename_stack_size += 10) * sizeof(char *));
    else
      node_filename_stack =
          (char **)xrealloc(node_filename_stack,
                            (node_filename_stack_size + 10) * sizeof(char *));
  }
...
char *filename_part(char *filename) {
  ...
  while (i && filename[i] != '/' && filename[i] != ':') i--;
  if (filename[i] == '/' || filename[i] == ':') i++;
  ...
  if (rindex(result, '.')) *(rindex(result, '.')) = '\0';
  return (result);
#else
...
char *expand_filename(char *filename, char *input_name) {
  ...
  while (i && result[i] != '/' && result[i] != ':') i--;
  if (result[i] == '/' || result[i] == ':') i++;
  ...
  if (filename && (initial_character = *filename)) {
    ...
    if (initial_character != '~') {
      ...
      if (filename[1] == '/') {
        ...
        temp_name = (char *)xmalloc(
            strlen(&filename[2]) + 1 + temp_home ? strlen(temp_home) : 0);
        ...
        if (!user_entry) return (savestring(filename));
...
char *read_token() {
  ...
  for (i = 0; ((input_text_offset != size_of_input_text) &&
               (character = curchar()) && command_char(character));
       i++, input_text_offset++)
    ;
...
void fix_whitespace(char *string) {
  ...
  while (string[string_index]) {
    ...
    if (c == ' ' || c == '\n' || c == '\t') {
      temp[temp_index - 1] = ' ';
      while ((c = string[string_index]) && (c == ' ' || c == '\t' || c == '\n'))
        string_index++;
    }
  }
  temp[temp_index] = '\0';
...
static char *suffixes[] = {"", ".texinfo", ".texi", ".txinfo", (char *)NULL};
...
void convert(char *name) {
  ...
  for (i = 0; suffixes[i]; i++) {
    ...
    if (find_and_load(filename)) break;
  ...
  if (!no_headers) {
    if (amiga_guide) {
      ...
      if (amiga_guide) add_word_args("\n@Width %d\n", fill_column);

      add_word_args(
          "\n\nThis is the AmigaGuide? file %s, produced by Makeinfo-%d.%d "
          "from ",
...
finished:
  ...
  if (output_stream != NULL) {
    ...
    if (tag_table != NULL) {
      ...
      if (!no_headers && !amiga_guide) write_tag_table();
    }
    ...
    if (validating) validate_file(real_output_filename, tag_table);

    /* This used to test  && !errors_printed.
       But some files might have legit warnings.  So split anyway.  */
    if (splitting) split_file(real_output_filename, 0);
  }
}
...
void reader_loop() {
  ...
  while (!done) {
    ...
    if (!in_fixed_width_font && (character == '\'' || character == '`') &&
    ...
    if (whitespace(character)) {
      ...
      while (i < size_of_input_text && whitespace(input_text[i])) i++;

      if (i == size_of_input_text || input_text[i] == '\n') {
        if (i == size_of_input_text) i--;
    ...
    if (character == '\n') {
      ...
      if (in_menu && input_text_offset + 1 < size_of_input_text) {
        ...
        if (amiga_guide && !no_headers) {
          if (amiga_guide_convert_menu()) continue;
        }
      }
    }
...
COMMAND *get_command_entry(char *string) {
  ...
  for (i = 0; CommandTable[i].name; i++)
    if (strcmp(CommandTable[i].name, string) == 0) return (&CommandTable[i]);
...
void read_command() {
  ...
  if (entry->argument_in_braces) remember_brace(entry->proc);

  (*(entry->proc))(START);
}

/* Return the string which invokes PROC; a pointer to a function. */
char *find_proc_name(FUNCTION *proc) {
  int i;

  for (i = 0; CommandTable[i].name; i++)
    if (proc == CommandTable[i].proc) return (CommandTable[i].name);
  return ("NO_NAME!");
}

void init_brace_stack() { brace_stack = (BRACE_ELEMENT *)NULL; }
...
void add_char(int character) {
  ...
  if (must_start_paragraph && character != '\n') {
    ...
    if (current_indent > output_column &&
        (!amiga_guide || !in_amiga_guide_button)) {
        ...
        if (filling_enabled) {
          ...
          while (--temp > 0 && output_paragraph[temp] != '\n') {
            ...
            if (output_paragraph[temp] == ' ') {
              ...
              while (temp && whitespace(output_paragraph[temp - 1])) temp--;
                 ...
                 them. */
              {
                ...
                for (;; t1++) {
                  if (t1 == output_paragraph_offset) {
                    if (whitespace(character)) suppress_insert = 1;
                    break;
                  }
                  if (!whitespace(output_paragraph[t1])) break;
                }
              ...
              if (indented_fill && current_indent) {
                int buffer_len =
                    ((output_paragraph_offset - temp) + current_indent);
                   ...
                   front of the wrap point. */
                {
                  ...
                  while (stack) {
                    if (stack->pos >= temp) stack->pos += current_indent;
                    stack = stack->next;
                  }
                }

                while (current_indent > 0 && indentation != current_indent)
                ...
                if (output_paragraph_offset + buffer_len >=
                  ...
                  unsigned char *tt = xrealloc(
                      output_paragraph, (paragraph_buffer_len += buffer_len));
                  output_paragraph = tt;
                }
                strncpy((char *)&output_paragraph[temp], temp_buffer,
                        buffer_len);
              ...
              output_column = 0;
              if (amiga_guide &&
                  !strncmp("@{", &output_paragraph[temp + current_indent], 2))
...
void inhibit_output_flushing() { flushing_ignored++; }

/* Allow calls to flush_output () to write the paragraph data. */
void uninhibit_output_flushing() { flushing_ignored--; }

void flush_output() {
  int i;

  if (!output_paragraph_offset || flushing_ignored) return;
...
void close_single_paragraph() { close_paragraph_with_lines(0); }
...
void close_paragraph() {
  ...
  if (paragraph_is_open && !must_start_paragraph) {
    ...
    if (force_flush_right) do_flush_right_indentation();
  ...
  if (output_paragraph[0] != '\n') {
    ...
    if (output_paragraph_offset < fill_column) {
      ...
      if (fill_column >= paragraph_buffer_len)
        output_paragraph =
            xrealloc(output_paragraph, (paragraph_buffer_len += fill_column));
...
void start_paragraph() {
  ...
  if (paragraph_is_open) close_paragraph();
  ...
  if (!must_start_paragraph) {
    ...
    if (!no_indent && (!amiga_guide || !in_amiga_guide_button)) {
      if (inhibit_paragraph_indentation) {
        ...
        if (inhibit_paragraph_indentation < 0) inhibit_paragraph_indentation++;
...
void indent(int amount) {
  ...
  while (elt) {
    if (elt->pos >= output_paragraph_offset) elt->pos += amount;
    elt = elt->next;
  }
...
int search_forward(char *string, int from) {
  ...
  while (from < size_of_input_text) {
    if (strncmp(input_text + from, string, len) == 0) return (from);
    from++;
  }
  return (-1);
}
...
char *insertion_type_names[] = {
    ...
    "cartouche",     "table",     "ftable",     "vtable",     "group",
    "ifinfo",        "flushleft", "flushright", "ifset",      "ifclear",
    "deffn",         "defun",     "defmac",     "defspec",    "defvr",
    "defvar",        "defopt",    "deftypefn",  "deftypefun", "deftypevr",
    "deftypevar",    "defcv",     "defivar",    "defop",      "defmethod",
...
void init_insertion_stack() { insertion_stack = (INSERTION_ELT *)NULL; }
...
void push_insertion(enum insertion_type type, char *item_function)
...
int defun_insertion(enum insertion_type type)

{
  return ((type == deffn) || (type == defun) || (type == defmac) ||
          (type == defspec) || (type == defvr) || (type == defvar) ||
          (type == defopt) || (type == deftypefn) || (type == deftypefun) ||
          (type == deftypevr) || (type == deftypevar) || (type == defcv) ||
          (type == defivar) || (type == defop) || (type == defmethod) ||
          (type == deftypemethod) || (type == deftp));
}
...
void stop_enumerating() {
  --enumstack_offset;
  if (enumstack_offset < 0) enumstack_offset = 0;
...
void begin_insertion(enum insertion_type type) {
  ...
  switch (type) {
    ...
    case format:
      ...
      if (type != format) current_indent += default_indentation_increment;
    ...
    case group:
      ...
      if (!insertion_stack->next || insertion_stack->next->insertion != example)
        close_single_paragraph();
      break;
...
void end_insertion(enum insertion_type type) {
  ...
  if (type != temp_type) {
    line_error("`%cend' expected `%s', but saw `%s'.", COMMAND_PREFIX,
  ...
  switch (type) {
    ...
    case menu:
      ...
      if (!no_headers) close_insertion_paragraph();
      break;
    ...
    case quotation:
      ...
      /* @quotation is the only one of the above without a fixed width
         font. */
      if (type != quotation) in_fixed_width_font--;
...
cm_bullet(int arg) {
  if (arg == START)
    ...
    {
      output_column -=
          (amiga_guide_hidden_chars = strlen("@{b}") + strlen("@{ub}"));
      add_word("@{b}*@{ub}");
    }
...
cm_today(int arg) {
  ...
  if (arg == START) {
    ...
    add_word_args("%d %s %d", (ts->tm_mday), (months[ts->tm_mon]),
                  ((ts->tm_year) + 1900));
  }
}
...
int cm_code(int arg) {
  ...
  /* If we are printing an index, or we are in an AmigaGuide button,
     simply return */
  if (in_amiga_guide_button || printing_index) return 0;
...
cm_file(int arg) { cm_code(arg); }

cm_kbd(int arg) { cm_code(arg); }
...
int cm_var(int arg, int start_pos, int end_pos) {
  ...
  if (!amiga_guide || amiga_guide_34) {
    ...
    if (printing_index) return 0;
    if (arg == START) amiga_set_attributes(AMIGA_VAR_ON);
    if (arg == END) amiga_set_attributes(AMIGA_VAR_OFF);
  }
}
...
cm_emph(int arg) {
  ...
  if (amiga_guide && (!amiga_guide_34)) {
    if (arg == START) amiga_set_attributes(AMIGA_EMPH_ON);

    if (arg == END) amiga_set_attributes(AMIGA_EMPH_OFF);
  } else
    add_char('*');
}

cm_strong(int arg, int position) { cm_emph(arg); }
...
cm_italic(int arg, int start, int end) {
  ...
  if (amiga_guide && (!amiga_guide_34)) {
    if (arg == START) amiga_set_attributes(AMIGA_ITALIC_ON);
    if (arg == END) amiga_set_attributes(AMIGA_ITALIC_OFF);
  }
}

/* Current text is highlighted. */
cm_bold(int arg, int start, int end) { cm_italic(arg, start, end); }
...
/* Italicize titles. */
cm_title(int arg, int start, int end) { cm_italic(arg, start, end); }
...
cm_obsolete(int arg, int start, int end) {
  if (arg == START) warning("The command `@%s' is obsolete", command);
}
...
void insert_and_underscore(int with_char) {
  ...
  for (i = 0; (i < len - amiga_hide_title_chars); i++) add_char(with_char);
  insert('\n');
  close_paragraph();
...
struct {
  char *name;
  int level;
} section_alist[] = {{"unnumberedsubsubsec", 5},
                     {"unnumberedsubsec", 4},
                     {"unnumberedsec", 3},
                     {"unnumbered", 2},
                     {"appendixsubsubsec", 5},
...
cm_top() {
  ...
  if (top_encountered) {
    ...
    if (tag_table) tag_table->flags |= IS_TOP;
    ...
    /* Now set the logical hierarchical level of the Top node. */
    {
      ...
      if (input_text_offset > 0) {
        ...
        if (input_text_offset < size_of_input_text) input_text_offset++;
...
void sectioning_underscore(char *command) {
  char character;
  int level;
...
write_tag_table() { return (write_tag_table_internal(0)); /* Not indirect. */ }

write_tag_table_indirect() { return (write_tag_table_internal(1)); }
...
normalize_node_name(char *string) {
  ...
  if (stricmp(string, "Top") == 0) strcpy(string, "Top");
}
...
TAG_ENTRY *find_node(char *name) {
  ...
  while (tag != (TAG_ENTRY *)NULL) {
    if (strcmp(tag->node, name) == 0) return (tag);
    tag = tag->next_ent;
  }
...
void remember_node(char *node, char *prev, char *next, char *up, int position,
                   int line_no, int no_warn) {
  /* Check for existence of this tag already. */
  if (validating) {
    TAG_ENTRY *tag = find_node(node);
    if (tag) {
      line_error("Node `%s' multiply defined (%d is first definition)", node,
                 tag->line_no);
      return;
    }
  }
...
cm_node() {
  ...
  if (strcmp(command, "nwnode") == 0) no_warn = 1;
  ...
  if (!no_headers) {
       ...
       considering the first node of the database. */
    {
      if (amiga_guide_first_node) {
        ...
        add_word_args("\n@EndNode\n\n");
        add_word_args("@Node \"%s\" \"%s/%s\"", node, pretty_output_filename,
                      node);
      }
    } else
  ...
  defaulting =
      ((strlen(next) == 0) && (strlen(prev) == 0) && (strlen(up) == 0));
  ...
  if (defaulting) {
    ...
    if (this_section < 0) {
      ...
      line_error("Node `%s' requires a sectioning command (e.g. @%s)", node,
                 polite_section_name);
    } else {
      if (stricmp(node, "Top") == 0) {
           ...
           this node, if there is a menu in this node. */
        {
          ...
          if (orig_size < 0) orig_size = size_of_input_text;
      ...
      while (ref) {
        if (ref->section == (this_section - 1) && ref->type == menu_reference &&
          ...
          while ((ref->section == this_section - 1) && (ref->next) &&
          ...
          } else if (!ref->next && stricmp(ref->containing_node, "Top") == 0) {
            free(prev);
  ...
  if (!no_headers) {
    if (amiga_guide) {
      ...
      if (*next) add_word_args(",  Next: %s", next);

      if (*prev) add_word_args(",  Prev: %s", prev);

      if (*up) add_word_args(",  Up: %s", up);
    }
  }
  ...
  if (this_section >= 0) current_section = this_section;
...
void validate_file(char *filename, TAG_ENTRY *tag_table)

{
  ...
  while (tags != (TAG_ENTRY *)NULL) {
      ...
      if (temp_tag = find_node(tags->next)) {
        ...
        if (temp_tag->flags & NO_WARN) {
          ...
          if (!prev || (strcmp(prev, tags->node) != 0)) {
            ...
            line_error("This node (`%s') is the one with the bad `Prev'",
                       temp_tag->node);
            input_filename = tags->filename;
            line_number = tags->line_no;
      ...
      if (!valid)
        ...
        if (tags->up && (strcmp(tags->prev, tags->up) != 0)) {
          ...
          if (!temp_tag || (temp_tag->flags & NO_WARN)) {
            ...
            if (!temp_tag->next || (strcmp(temp_tag->next, tags->node) != 0)) {
              line_error("Node `%s''s Prev field not pointed back to",
                         tags->node);
              line_number = temp_tag->line_no;
              input_filename = temp_tag->filename;
              line_error("This node (`%s') is the one with the bad `Next'",
                         temp_tag->node);
              input_filename = tags->filename;
              line_number = tags->line_no;
    ...
    if (!tags->up && (stricmp(tags->node, "Top") != 0))
      ...
      if (valid && *tags->up != '(') {
        ...
        for (;;) {
          if (!(nref = find_node_reference(tags->node, list))) break;
        ...
        if (!nref) {
          ...
          if (!tref)
            line_error(
                "`%s' has an Up field of `%s', but `%s' has no menu item for "
                "`%s'",
  ...
  while (tags != (TAG_ENTRY *)NULL) {
    ...
    if (tags->touched > reference_warning_limit) {
      input_filename = tags->filename;
      line_number = tags->line_no;
      warning("Node `%s' has been referenced %d times", tags->node,
              tags->touched);
    }
...

{
  ...
  if (!result) {
    line_number = line;
    line_error(
        "Validation error.  `%s' field points to node `%s', which doesn't "
        "exist",
        label, tag);
    return (0);
  }
    ...
    while (tags) {
      ...
      for (; tags; tags = tags->next_ent) {
        ...
        if (tags->next_ent->position > limit) {
          if (tags->position == file_top) tags = tags->next_ent;
          file_bot = tags->position;
        write_region : {
          int fd;
          char *split_filename;

          split_filename = (char *)xmalloc(10 + strlen(root_pathname) +
                                           strlen(root_filename));
          sprintf(split_filename, "%s%s-%d", root_pathname, root_filename,
                  which_file);

          fd = open(split_filename, O_WRONLY | O_TRUNC | O_CREAT, 0666);

          if ((fd < 0) || (write(fd, the_header, header_size) != header_size) ||
...
char *reftype_type_string(enum reftype type) {
  switch (type) {
    case menu_reference:
      return ("Menu");
...
NODE_REF *find_node_reference(char *node, NODE_REF *ref_list)

{
  while (ref_list) {
    if (strcmp(node, ref_list->node) == 0) break;
    ref_list = ref_list->next;
  }
  return (ref_list);
}
...
#define menu_starter "* "
char *glean_node_from_menu(int remember_reference) {
  int i, orig_offset = input_text_offset;
  char *nodename;

  if (strncmp(&input_text[input_text_offset + 1], menu_starter,
...
save_node:
  ...
  i = strlen(nodename);
  if (i && nodename[i - 1] == ':') nodename[i - 1] = '\0';
...
static void isolate_nodename(char *nodename) {
  ...
  for (; c = nodename[i]; i++) {
    ...
    if (c == '\t' || c == '\n' || c == ',' ||
        ((paren_seen && nodename[i - 1] == ')') && (c == ' ' || c == '.')) ||
        (c == '.' &&
         ((!nodename[i + 1] || (cr_or_whitespace(nodename[i + 1])) ||
           (nodename[i + 1] == ')')))))
      break;
  }
  nodename[i] = '\0';
...
int amiga_guide_convert_menu() {
  ...
  if (curchar() == ':') {
    ...
    output_column -= (strlen("@{\"\" Link \"\"}") + strlen(gadgetname));
    execute_string("\n@w{ @@@{\" %s \" Link \"%s\"@} }", gadgetname,
                   gadgetname);
   ...
   been counted.
    ...
    if (*nodename == '(') {
      ...
      in_amiga_guide_button = 1;
      output_column -= (strlen("@{\"\" Link \"/\"}") + strlen(nodename + 1) +
                        strlen(nodename + i + 1));
      execute_string("\n@w{ @@@{\" %s \" Link \"%s/%s\"@} }", gadgetname,
                     nodename + 1, nodename + i + 1);
      in_amiga_guide_button = 0;

    } else {
      in_amiga_guide_button = 1;
      output_column -= (strlen("@{\"\" Link \"\"}") + strlen(nodename));
      execute_string("\n@w{ @@@{\" %s \" Link \"%s\"@} }", gadgetname,
                     nodename);
      in_amiga_guide_button = 0;
    }
    ...
    if (*arg5 || *arg4) {
      ...
      if (amiga_guide) {
        if (no_headers) {
          ...
          in_amiga_guide_button = 1;
          output_column -=
              (amiga_guide_hidden_chars =
                   strlen("@{\"\" Link \"/\"}") + strlen(arg4) + strlen(arg1));
          canon_white(node_name);
          execute_string("@w{@@@{\"%s\" Link \"%s/%s\"@}}", node_name, arg4,
                         arg1);
          in_amiga_guide_button = 0;
        }
      } else
  ...
  if (arg == START) {
    ...
    if (amiga_guide) {
      ...
      if (no_headers) {
        ...
        output_column -=
            (amiga_guide_hidden_chars =
                 strlen("@{\"\" Link \"/\"}") + strlen(file) + strlen(node));
        canon_white(pname);
...
cm_quotation() { begin_insertion(quotation); }

cm_example() { begin_insertion(example); }

cm_smallexample() { begin_insertion(smallexample); }

cm_lisp() { begin_insertion(lisp); }

cm_smalllisp() { begin_insertion(smalllisp); }

/* @cartouche/@end cartouche draws box with rounded corners in
   TeX output.  Right now, just a NOP insertion. */
cm_cartouche() { begin_insertion(cartouche); }

cm_format() { begin_insertion(format); }

cm_display() { begin_insertion(display); }

cm_itemize() { begin_insertion(itemize); }

cm_enumerate() { do_enumeration(enumerate, "1"); }
...
void do_enumeration(int type, char *default_string)
...
cm_table() { begin_insertion(table); }

cm_ftable() { begin_insertion(ftable); }

cm_vtable() { begin_insertion(vtable); }

cm_group() { begin_insertion(group); }

cm_ifinfo() { begin_insertion(ifinfo); }

/* Begin an insertion where the lines are not filled or indented. */
cm_flushleft() { begin_insertion(flushleft); }

/* Begin an insertion where the lines are not filled, and each line is
   forced to the right-hand side of the page. */
cm_flushright() { begin_insertion(flushright); }
...
cm_set() { handle_variable(SET); }

/* Remove a variable whose name appears as the first word on this line. */
cm_clear() { handle_variable(CLEAR); }

cm_ifset() { handle_variable(IFSET); }

cm_ifclear() { handle_variable(IFCLEAR); }
...
void handle_variable_internal(int action, char *name) {
  char *temp;
  ...
  if (*temp) additional_text_present++;
          ...
          if ((action == IFSET && !set_p(name)) ||
            ...
            while (!done) {
              ...
              if (*line == COMMAND_PREFIX &&
                ...
                while (*cname && whitespace(*cname)) cname++;
                temp = cname;

                while (*temp && !whitespace(*temp)) temp++;
                *temp = '\0';
...
void initialize_token_accumulator(struct token_accumulator *accumulator) {
  (accumulator->length) = 0;
  (accumulator->index) = 0;
  (accumulator->tokens) = NULL;
}

void accumulate_token(struct token_accumulator *accumulator, char *token) {
...
char *copy_substring(char *start, char *end) {
  ...
  while (scan < end) *scan_result++ = *scan++;
...
int scan_group_in_string(char **string_pointer) {
  int c;
  char *scan_string;
...
char **args_from_string(char *string) {
  ...
  while ((*scan_string) != '\0') {
    ...
    if (whitespace(*scan_string)) {
      scan_string += 1;
      while (whitespace(*scan_string)) scan_string += 1;
    ...
    /* Otherwise a token is delimited by whitespace, parentheses,
      ...
      while (1) {
        ...
        if (!c || (whitespace(c) || DEFUN_SELF_DELIMITING(c) || c == '{' ||
                   c == '}')) {
          scan_string--;
          break;
        }
    ...
    accumulate_token(&accumulator, copy_substring(token_start, token_end));
  }
  accumulate_token(&accumulator, NULL);
  return (accumulator.tokens);
}

void process_defun_args(char **defun_args, int auto_var_p) {
  int pending_space = 0;

  while (1) {
    char *defun_arg = *defun_args++;

    if (defun_arg == NULL) break;
...
char *next_nonwhite_defun_arg(char ***arg_pointer) {
  char **scan = (*arg_pointer);
  char *arg = (*scan++);

  if ((arg != 0) && (*arg == ' ')) arg = *scan++;
...
void defun_internal(enum insertion_type type, int x_p) {
  ...
  if ((base_type == deftypefn) || (base_type == deftypevr) ||
      (base_type == defcv) || (base_type == defop) ||
      (base_type == deftypemethod))
...
cm_defun() {
  ...
  if (x_p && (!insertion_level || insertion_stack->insertion != type)) {
    line_error("Must be in a `%s' insertion in order to use `%s'x", command,
               command);
    discard_until("\n");
    return 0;
  }
...
cm_end() {
  ...
  if (strlen(temp) == 0) line_error("`@%s' needs something after it", command);

  type = find_type_from_name(temp);

  if (type == bad_type) {
    line_error("Bad argument to `%s', `%s', using `%s'", command, temp,
               insertion_type_pname(current_insertion_type()));
  }
  end_insertion(type);
  free(temp);
...
cm_noindent() {
  if (!inhibit_paragraph_indentation) inhibit_paragraph_indentation = -1;
}
...
cm_center() {
  ...
  if (length < (fill_column - fudge_factor)) {
    ...
    for (i = 0; i < length; i++) insert(line[i]);
...
/* An error signaled. */
cm_error(arg) int arg;
{
  if (arg == END) add_word("error-->");
}

/* The location of point in an example of a buffer. */
cm_point(arg) int arg;
{
  if (arg == END) add_word("-!-");
}
...
cm_exdent() {
  ...
  if (current_indent) current_indent -= default_indentation_increment;
...
void init_indices() {
  ...
  if (!the_indices) {
    the_indices =
        (INDEX_ELT **)xmalloc((1 + defined_indices) * sizeof(INDEX_ELT *));
    the_indices[defined_indices] = (INDEX_ELT *)NULL;

    name_index_alist =
        (INDEX_ALIST **)xmalloc((1 + defined_indices) * sizeof(INDEX_ALIST *));
    name_index_alist[defined_indices] = (INDEX_ALIST *)NULL;
  }

  /* If there were existing indices, get rid of them now. */
  for (i = 0; i < defined_indices; i++) undefindex(name_index_alist[i]->name);
...
int find_index_offset(char *name) {
  ...
  for (i = 0; i < defined_indices; i++)
    if (name_index_alist[i] && strcmp(name, name_index_alist[i]->name) == 0)
...
void defindex(char *name, int code) {
  int i, slot;
  ...
  if (slot < 0) {
    ...
    name_index_alist =
        (INDEX_ALIST **)xrealloc((char *)name_index_alist,
                                 (1 + defined_indices) * sizeof(INDEX_ALIST *));
    the_indices = (INDEX_ELT **)xrealloc(
        (char *)the_indices, (1 + defined_indices) * sizeof(INDEX_ELT *));
  }
...
cm_defindex() { gen_defindex(0); }

cm_defcodeindex() { gen_defindex(1); }
...
INDEX_ELT *index_append(INDEX_ELT *head, INDEX_ELT *tail) {
...
cm_synindex() {
  ...
  if (redirector < 0 || redirectee < 0) {
    ...
    name_index_alist[redirectee]->index = name_index_alist[redirector]->index;
  }
}
...
INDEX_ELT **sort_index(INDEX_ELT *index) {
    ...
    if (amiga_guide) {
      ...
      if (no_headers) {
        ...
        if (index->code) {
          output_column -=
              (amiga_guide_hidden_chars =
                   strlen("@{\"\" Link \"\"}") + strlen(index->node));
          execute_string("@w{ @@@{\" @code{%s} \" Link \"%s\"@} }",
                         index->entry, index->node);
        } else {
          output_column -=
              (amiga_guide_hidden_chars =
                   strlen("@{\"\" Link \"\"}") + strlen(index->node));
          execute_string("@w{ @@@{\" %s \" Link \"%s\"@} }", index->entry,
                         index->node);
        }
        in_amiga_guide_button = 0;
      }
      ...
      if (fill_column > 40 && output_column < 40) indent(40 - output_column);
...
void define_user_command(char *name, FUNCTION *proc, int needs_braces_p) {
  ...
  user_command_array = (COMMAND **)xrealloc(
      user_command_array, (1 + user_command_array_len) * sizeof(COMMAND *));
...
int set_paragraph_indent(char *string) {
  if (strcmp(string, "asis") == 0)
    ...
    if (sscanf(string, "%d", &paragraph_start_indent) != 1)
      ...
      if (paragraph_start_indent == 0) paragraph_start_indent = -1;
    }
  }
  return (0);
}
...
int set_footnote_style(char *string) {
  if ((stricmp(string, "separate") == 0) || (stricmp(string, "MN") == 0))
    footnote_style = SeparateNode;
  else if ((stricmp(string, "end") == 0) || (stricmp(string, "EN") == 0))
  ...
  switch (footnote_style) {
    case SeparateNode:
      ...
      if (first_footnote_this_node) {
        ...
        temp_string = (char *)xmalloc((strlen(current_node)) +
                                      (strlen("-Footnotes")) + 1);
        if (amiga_guide) {
          if (no_headers) {
            char *Footnotestring;
...
void add_macro(char *name, char *definition, char *filename, int lineno)
...
/* Delete the macro with name NAME.  The macro is deleted from the list,
   ...
   returned. */
MACRO_DEF *delete_macro(char *name) {
  int i;
  MACRO_DEF *def;
...
char *extract_colon_unit(char *string, int *index) {
  ...
  if (!string || (i >= strlen(string))) return ((char *)NULL);
...
char *get_file_info_in_path(char *filename, char *path, struct stat *finfo) {
  char *dir;
  int result, index = 0;
...
void amiga_set_attributes(char *attr_type) {
  int after_attr = 0;
  if (in_amiga_guide_button || printing_index || !amiga_guide_39) return;
  ...
  if (no_headers) {
    ...
    if (after_attr) output_column -= (amiga_guide_hidden_chars = 4);
    after_attr = 0;
    if (in_amiga_guide_title) amiga_hide_title_chars += 4;
    ...
    if (input_text[input_text_offset - 1] == '\\') add_word(" ");
    ...
    if (in_amiga_guide_title) amiga_hide_title_chars += strlen(attr_type);
  }
}
